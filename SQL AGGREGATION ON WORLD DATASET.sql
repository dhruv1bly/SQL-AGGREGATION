...
SELECT co.Continent, COUNT(cl.Language) AS OfficialLanguageCount
FROM country co
...
SELECT co.Name, AVG(ci.Population) AS AvgCityPop
FROM country co
...
#Question 8 : List continents where the average city population is greater than 200,000.alter
SELECT co.Continent
FROM country co
JOIN city ci ON co.Code = ci.CountryCode
GROUP BY co.Continent
